...
import { deletePaymentProofs } from '@/lib/local-payment-proofs'
...
export async function DELETE(request:Request,context:{params:Promise<{id:string}>}){
 if(!strictSameOrigin(request))return publicApiHeaders(apiError('Invalid request origin',403));if(!checkRateLimit(request,'public-payment-delete',20).allowed)return publicApiHeaders(apiError('Too many requests',429))
 ...
 try{const payment=(await listPayments()).find(p=>p.id===id);if(!payment)return publicApiHeaders(apiOk({deleted:true,alreadyDeleted:true}));if(!verifyPaymentDeleteCapability(capability,payment.publicDeleteTokenHash))return publicApiHeaders(apiError('This device is not authorised to delete that payment',403));if(payment.status!=='Pending')return publicApiHeaders(apiError('Only an exact Pending submission can be deleted',409));const result=await deletePendingPublicPayment(id);if(result.outcome!=='deleted')return publicApiHeaders(apiError('Payment is no longer Pending',409));await deletePaymentProofs(id);await removePaymentNotifications(id);return publicApiHeaders(apiOk({deleted:true}))}catch(e){return publicApiHeaders(apiError(e instanceof Error?e.message:'Could not delete payment',500))}
}
