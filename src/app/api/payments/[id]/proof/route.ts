...
export async function GET(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  if(proof?.key.startsWith('payments/'))return NextResponse.redirect(createR2ViewUrl(proof.key),302)
  ...
  return new NextResponse(bytes,{headers:{'Content-Type':proof.contentType,'Content-Length':String(bytes.byteLength),'Content-Disposition':`inline; filename*=UTF-8''${encodeURIComponent(proof.name)}`,'Cache-Control':'private, no-store, max-age=0','X-Content-Type-Options':'nosniff','Referrer-Policy':'no-referrer'}})
}
