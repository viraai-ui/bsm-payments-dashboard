...
export function PaymentProofViewer({ proofs, initial = 0, onClose }: { proofs: ViewerProof[]; initial?: number; onClose: () => void }) {
  ...
  useEffect(() => {
    const key = (event: KeyboardEvent) => {
      ...
      if ((event.key === '+' || event.key === '=') && proof?.contentType !== 'application/pdf') {
        setZoom(value => Math.min(MAX_ZOOM, value + ZOOM_STEP))
      }
      if (event.key === '-' && proof?.contentType !== 'application/pdf') {
  ...
  }, [proofs.length, onClose, proof?.contentType])
  ...
  return createPortal(
    <div className="proof-viewer-backdrop" role="dialog" aria-modal="true" aria-label="Payment proof viewer" onMouseDown={event => { if (event.target === event.currentTarget) onClose() }}>
      <section className="proof-viewer" onTouchStart={event => { touch.current = event.touches[0].clientX }} onTouchEnd={event => {
        ...
        <header>
          ...
          <button type="button" onClick={onClose} aria-label="Close proof viewer">×</button>
        </header>
