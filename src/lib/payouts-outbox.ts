...
export async function reconcilePaymentOutbox(){const payments=(await readLocalJsonFresh<{payments:Payment[]}>('payments.json',{payments:[]})).payments;let added=0;for(const payment of payments){if(!payment.salesOrderId||!payment.salesOrderNumber)continue;const audit=[...(payment.audit||[])].reverse().find(entry=>entry.type==='claimed'||entry.type==='voided'||entry.type==='edited'||(entry.type==='status_changed'&&(entry.to==='Payment Received'||entry.from==='Payment Received')));if(!audit)continue;const type:PaymentEventType=audit.type==='voided'?'voided':audit.type==='edited'?'updated':audit.type==='claimed'||audit.to==='Payment Received'?'received':'reversed';const snapshot={...payment,updatedAt:audit.at};const before=await listOutbox();const id=eventIdFor(payment.id,type,audit.at);if(before.some(item=>item.eventId===id))continue;await enqueuePaymentEvent(snapshot,type,async()=>({id:payment.salesOrderId!,salesOrderNumber:payment.salesOrderNumber!,customerName:payment.customerName,currency:'INR'}));added++}return added}
export async function processDueOutbox(fetcher:typeof fetch=fetch,now=new Date(),limit=25){await reconcilePaymentOutbox();const store=await readLocalJsonFresh(FILE,EMPTY),due=store.events.filter(item=>!['delivered','failed'].includes(item.status)&&((item.status==='delivering'&&(!item.leaseExpiresAt||Date.parse(item.leaseExpiresAt)<=now.getTime()))||(['pending','retrying'].includes(item.status)&&(!item.nextAttemptAt||Date.parse(item.nextAttemptAt)<=now.getTime())))).slice(0,limit),results=[] as OutboxItem[];for(const item of due){const result=await deliverOutboxEvent(item.eventId,fetcher,now);if(result)results.push(result)}return results}
