...
/** Security-sensitive reads never use a bundled file or stale cache remotely. */
export async function readAuthoritativeJson<T>(filename:string,fallback:T):Promise<T>{const selected=backend();if(selected==='local')return localRead(filename,fallback);try{const object=selected==='github'?await getGitHubDataObject(`data/${filename}`):await getR2Object(objectKey(filename));return object?parse<T>(object.bytes,filename):fallback}catch(error){throw new Error('Authoritative data store is temporarily unavailable',{cause:error})}}
/** Bypass the process-local read cache for live synchronization endpoints. */
export async function readLocalJsonFresh<T>(filename:string,fallback:T):Promise<T>{const selected=backend();if(selected==='local')return localRead(filename,fallback);const stored=await authoritativeRemoteRead(selected,filename,fallback);cacheValue(filename,stored);return stored.data}
