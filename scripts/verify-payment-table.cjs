...
const ui=fs.readFileSync(path.join(__dirname,'../src/components/PaymentsClient.tsx'),'utf8');assert.doesNotMatch(ui,/role === "Viewer" \? ["']Payment["'] : ["']Receipt["']/);assert.match(ui,/money\(p\.paymentAmount\)/);assert.match(ui,/paymentOutstandingById\(payments\)/);assert.match(ui,/salespersonName/);assert.match(ui,/setForm\(emptyForm\(\)\).*submissionKey\.current=""/s);assert.equal((ui.match(/filtered\.map\(\(p\) =>/g)||[]).length,2,'desktop and mobile consume the identical sorted read model');assert.match(ui,/<PendingList orders=\{visiblePending\}/,'pending grid and list consume the same order')
assert.match(ui,/function DesktopRow[\s\S]*?<StatusControl/);assert.match(ui,/function MobileCard[\s\S]*?<StatusControl/);assert.match(ui,/<option value="Pending">Payment Pending<\/option>/);assert.match(ui,/role === "Admin" \|\| role === "Accounts"/);assert.match(ui,/Change this received payment back to Payment Pending\?/)
